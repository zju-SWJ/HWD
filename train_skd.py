...
from losses.loss import SegCrossEntropyLoss, CriterionKD, CriterionAdv, CriterionAdvForG, CriterionAdditionalGP, CriterionPairWise
    ...
    def train(self):
        ...
        for iteration, (images, targets, _) in enumerate(self.train_loader):
            ...
            if self.args.no_adv == False:
                ...
                losses = task_loss + kd_loss + pairwise_loss
            ...
            if iteration % log_per_iters == 0 and save_to_disk:
                if self.args.no_adv == False:
                    ...
                    logger.info("Iters: {:d}/{:d} || Lr: {:.6f} || Task Loss: {:.4f} || KD Loss: {:.4f}" \
                        "|| PairWise Loss: {:.4f} || Cost Time: {} || Estimated Time: {}".format(
                        iteration, self.args.max_iterations, self.optimizer.param_groups[0]['lr'], task_losses_reduced.item(),
                        kd_losses_reduced.item(), pairwise_losses_reduced.item(),
                        str(datetime.timedelta(seconds=int(time.time() - start_time))), eta_string))
