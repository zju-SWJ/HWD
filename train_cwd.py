...
from losses.loss import SegCrossEntropyLoss, CriterionCWD
...
def parse_args():
    ...
    parser.add_argument("--lambda-logit", type=float, default=3.0, help="lambda_logit")
    parser.add_argument("--lambda-feat", type=float, default=0.0, help="lambda_feat")
...
class Trainer(object):
    def __init__(self, args):
        ...
        self.criterion_cwd = CriterionCWD(args.temperature, BatchNorm2d).to(self.device)

        self.optimizer = torch.optim.SGD(self.s_model.parameters(), lr=args.lr, momentum=args.momentum, weight_decay=args.weight_decay)
        self.optimizer.add_param_group({'params':self.criterion_cwd.parameters(), 'initial_lr': args.lr})

        if args.distributed:
            self.s_model = nn.parallel.DistributedDataParallel(self.s_model, device_ids=[args.local_rank], output_device=args.local_rank)
            self.criterion_cwd = nn.parallel.DistributedDataParallel(self.criterion_cwd, device_ids=[args.local_rank], output_device=args.local_rank)
    ...
    def train(self):
        ...
        for iteration, (images, targets, _) in enumerate(self.train_loader):
            ...
            cwd_logit_loss = self.args.lambda_logit * self.criterion_cwd(s_outputs[0], t_outputs[0], False)
            cwd_feat_loss = self.args.lambda_feat * self.criterion_cwd(s_outputs[-1], t_outputs[-1], True)

            losses = task_loss + cwd_logit_loss + cwd_feat_loss
            ...
            cwd_logit_losses_reduced = self.reduce_mean_tensor(cwd_logit_loss)
            cwd_feat_losses_reduced = self.reduce_mean_tensor(cwd_feat_loss) 

            eta_seconds = ((time.time() - start_time) / iteration) * (self.args.max_iterations - iteration)
            eta_string = str(datetime.timedelta(seconds=int(eta_seconds)))

            if iteration % log_per_iters == 0 and save_to_disk:
                logger.info("Iters: {:d}/{:d} || Lr: {:.6f} || Task Loss: {:.4f} || Logit Loss: {:.4f}" \
                    "|| Feat Loss: {:.4f} || Cost Time: {} || Estimated Time: {}".format(
                        iteration, self.args.max_iterations, self.optimizer.param_groups[0]['lr'], task_losses_reduced.item(),
                        cwd_logit_losses_reduced.item(), cwd_feat_losses_reduced.item(),
...
def save_checkpoint(model, args, is_best=False):
    ...
    filename = 'cwd_{}_{}_{}.pth'.format(args.student_model, args.student_backbone, args.dataset)
    ...
    if is_best:
        best_filename = 'cwd_{}_{}_{}_best_model.pth'.format(args.student_model, args.student_backbone, args.dataset)
...
if __name__ == '__main__':
    ...
    logger = setup_logger("semantic_segmentation", args.log_dir, get_rank(), 
        filename='cwd_{}_{}_{}_log.txt'.format(args.student_model, args.student_backbone, args.dataset))
