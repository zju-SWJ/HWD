...
from losses.memory_loss import StudentSegContrast
...
class Trainer(object):
    def __init__(self, args):
        ...
        self.criterion_memory_contrast.eval()
    
        self.optimizer = torch.optim.SGD(self.s_model.parameters(), lr=args.lr, momentum=args.momentum, weight_decay=args.weight_decay)
        self.optimizer.add_param_group({'params':self.criterion_memory_contrast.parameters(), 'initial_lr': args.lr})

        if args.distributed:
            self.s_model = nn.parallel.DistributedDataParallel(self.s_model, device_ids=[args.local_rank], output_device=args.local_rank)
            self.criterion_memory_contrast = nn.parallel.DistributedDataParallel(self.criterion_memory_contrast, device_ids=[args.local_rank], output_device=args.local_rank)
    ...
    def train(self):
        ...
        self.s_model.train()
        self.criterion_memory_contrast.train()
        for iteration, (images, targets, _) in enumerate(self.train_loader):
            ...
            minibatch_pixel_contrast_loss = self.args.lambda_minibatch_pixel * self.criterion_minibatch(s_outputs[-1], t_outputs[-1])
            memory_pixel_contrast_loss, memory_region_contrast_loss = self.criterion_memory_contrast(s_outputs[-1], t_outputs[-1], targets)
            ...
            losses = task_loss + kd_loss + minibatch_pixel_contrast_loss + memory_pixel_contrast_loss + memory_region_contrast_loss
            ...
            if iteration % log_per_iters == 0 and save_to_disk:
                logger.info("Iters: {:d}/{:d} || Lr: {:.6f} || Task Loss: {:.4f} || KD Loss: {:.4f}" \
                    "|| Mini-batch p2p Loss: {:.4f} || Memory p2p Loss: {:.4f} || Memory p2r Loss: {:.4f} " \
                        ...
                        kd_losses_reduced.item(), minibatch_pixel_contrast_loss_reduced.item(),
                        memory_pixel_contrast_loss_reduced.item(), memory_region_contrast_loss_reduced.item(),
...
def save_checkpoint(model, args, is_best=False):
    ...
    filename = 'cirkd_{}_{}_{}.pth'.format(args.student_model, args.student_backbone, args.dataset)
...
if __name__ == '__main__':
    ...
    logger = setup_logger("semantic_segmentation", args.log_dir, get_rank(), 
        filename='cirkd_{}_{}_{}_log.txt'.format(args.student_model, args.student_backbone, args.dataset))
