...
class COCOTrainSet(data.Dataset):
    def __init__(self, root, list_path, max_iters=None, crop_size=(512, 512), scale=True, mirror=True, ignore_label=-1):
        ...
        print('{} images are loaded!'.format(len(self.img_ids)))
        self.id_to_trainid = {255: ignore_label}
        self.num_class = 182 # actual number is 171
...
class COCOTestSet(data.Dataset):
    def __init__(self, root, list_path):
        self.root = root
        self.list_path = list_path
        self.img_ids = [i_id.strip().split() for i_id in open(list_path)]
