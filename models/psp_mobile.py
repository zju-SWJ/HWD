...
class PSPNet(nn.Module):
    def __init__(self, nclass, local_rank=None, pretrained_base='None', **kwargs):
...
def get_psp_mobile(local_rank=None,  pretrained='None', pretrained_base='None', num_class=19, **kwargs):
    model = PSPNet(num_class, local_rank=local_rank, pretrained_base=pretrained_base, **kwargs)
    if pretrained != 'None':
...
if __name__ == '__main__':
    model = get_psp_mobile()
    img = torch.randn(2, 3, 512, 512)
    output = model(img)
    print(output[-1].size())
    # (2, 128, 64, 64)
    # from utils import cal_param_size, cal_multi_adds
    # print('Params: %.2fM, Multi-adds: %.3fM' % (cal_param_size(model) / 1e6, cal_multi_adds(model, (2, 3, 512, 512)) / 1e6))
