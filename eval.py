...
from dataset.datasets import CSValSet, VOCDataValSet, ADEValSet, COCOValSet
...
def parse_args():
    ...
    parser.add_argument('--method', type=str, default='cirkd', help='method name') 
    ...
    # checkpoint and log
    parser.add_argument('--pretrained', type=str, default='./ckpt/deeplabv3_resnet101_citys_best_model.pth', help='pretrained seg model')
    parser.add_argument('--save-dir', default='./outputs/', help='Directory for saving predictions')
    parser.add_argument('--save-pred', action='store_true', default=False, help='save predictions')
    ...
    parser.add_argument('--flip-eval', action='store_true', default=False, help='flip_evaluation')
...
class Evaluator(object):
    def __init__(self, args, num_gpus):
        ...
        if args.dataset == 'citys':
            self.val_dataset = CSValSet(DATA_CS, DATALIST_CS_VAL)
        elif args.dataset == 'voc':
            self.val_dataset = VOCDataValSet(DATA_VOC, DATALIST_VOC_VAL)
        elif args.dataset == 'ade20k':
            self.val_dataset = ADEValSet(DATA_ADE20K, DATALIST_ADE20K_VAL)
        elif args.dataset == 'coco':
            self.val_dataset = COCOValSet(DATA_COCO, DATALIST_COCO_VAL)

        val_sampler = make_data_sampler(self.val_dataset, False, args.distributed)
        val_batch_sampler = make_batch_data_sampler(val_sampler, images_per_batch=1)
        self.val_loader = data.DataLoader(dataset=self.val_dataset, batch_sampler=val_batch_sampler,
                                          num_workers=args.workers, pin_memory=True)

        # create network
        BatchNorm2d = nn.SyncBatchNorm if args.distributed else nn.BatchNorm2d

        self.model = get_segmentation_model(model=args.model, backbone=args.backbone, aux=args.aux, 
                                            pretrained=args.pretrained, pretrained_base='None',
                                            local_rank=args.local_rank, norm_layer=BatchNorm2d,
                                            num_class=self.val_dataset.num_class).to(self.device)

        self.model.eval()
        with torch.no_grad():
            logger.info('Params: %.2fM FLOPs: %.2fG' % (cal_param_size(self.model) / 1e6, cal_multi_adds(self.model, (1, 3, 512, 512))/1e9))
    ...
    def eval(self):
        ...
        for i, (image, target, filename) in enumerate(self.val_loader):
            ...
            logger.info("Sample: {:d}, validation pixAcc: {:.3f}, mIoU: {:.3f}".format(i + 1, pixAcc * 100, mIoU * 100))
...
if __name__ == '__main__':
    ...
    logger = setup_logger("semantic_segmentation", args.save_dir, get_rank(), 
                filename='{}_{}_{}_{}_val.txt'.format(
                    args.model, args.backbone, args.dataset, args.method), mode='a+')
