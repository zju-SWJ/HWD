...
from losses.loss import SegCrossEntropyLoss, CriterionDIST
...
class Trainer(object):
    def __init__(self, args):
        ...
        self.criterion_dist = CriterionDIST().to(self.device)
    ...
    def train(self):
        ...
        for iteration, (images, targets, _) in enumerate(self.train_loader):
            ...
            dist_loss = self.criterion_dist(s_outputs[0], t_outputs[0])

            losses = task_loss + dist_loss
            ...
            dist_losses_reduced = self.reduce_mean_tensor(dist_loss)

            eta_seconds = ((time.time() - start_time) / iteration) * (self.args.max_iterations - iteration)
            eta_string = str(datetime.timedelta(seconds=int(eta_seconds)))

            if iteration % log_per_iters == 0 and save_to_disk:
                logger.info("Iters: {:d}/{:d} || Lr: {:.6f} || Task Loss: {:.4f} || DIST Loss: {:.4f}" \
                    " || Cost Time: {} || Estimated Time: {}".format(
                        ...
                        dist_losses_reduced.item(), str(datetime.timedelta(seconds=int(time.time() - start_time))), eta_string))
...
def save_checkpoint(model, args, is_best=False):
    ...
    filename = 'dist_{}_{}_{}.pth'.format(args.student_model, args.student_backbone, args.dataset)
    ...
    if is_best:
        best_filename = 'dist_{}_{}_{}_best_model.pth'.format(args.student_model, args.student_backbone, args.dataset)
...
if __name__ == '__main__':
    ...
    logger = setup_logger("semantic_segmentation", args.log_dir, get_rank(), 
        filename='dist_{}_{}_{}_log.txt'.format(args.student_model, args.student_backbone, args.dataset))
